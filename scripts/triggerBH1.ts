/**
 * triggerBH1.ts — Sentry event trigger and http.method tag verification script
 *
 * This script drives the BH1 bug (null-coupon order processing) and verifies that
 * the resulting Sentry HTTP incident carries the http.method tag. Without this tag,
 * the incident silently misroutes to Remedy's command harness.
 *
 * Exit code 0 ONLY when:
 * - At least EVENT_COUNT (default 10) requests fired to the bug endpoint
 ...
 *
 * Environment variables (all optional, but script will warn and exit non-zero if creds missing):
 * - TARGET_URL: deployed backend base URL (e.g., https://xxx.railway.app). REQUIRED.
 * - SENTRY_AUTH_TOKEN: Sentry API authentication token. If missing, tag verification skipped.
 * - SENTRY_ORG: Sentry organization slug. If missing, tag verification skipped.
 * - SENTRY_PROJECT: Sentry project slug. If missing, tag verification skipped.
 * - EVENT_COUNT: number of events to fire (default: 10). Script fires at least this many.
...
if (!TARGET_URL) {
  ...
  console.error("   Usage: TARGET_URL=https://xxx.railway.app bun run scripts/triggerBH1.ts");
  process.exit(1);
}
...
/**
 * Fire HTTP 500 events by requesting the BH1 bug endpoint.
 * Returns the count of HTTP 500 responses received.
 */
async function fireEvents(count: number): Promise<number> {
  console.log(`🔥 Firing ${count} requests to ${TARGET_URL}/orders/ord_null_coupon...`);
  ...
  for (let i = 0; i < count; i++) {
    try {
      const response = await fetch(`${TARGET_URL}/orders/ord_null_coupon`, {
        method: "GET",
        headers: {
          "User-Agent": "triggerBH1/1.0",
        },
      });
      ...
      if (response.status === 500) {
        ...
        console.log(
          `  [${i + 1}/${count}] ⚠ HTTP ${response.status} (expected 500)`,
        );
      }
    } catch (err) {
      const message =
      ...
      console.error(
        `  [${i + 1}/${count}] ✗ Request failed: ${message}`,
      );
      errorCount++;
    }
...
/**
 * Query Sentry API for the latest event and check for http.method tag.
...
async function verifyHttpMethodTag(): Promise<boolean> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Sentry API returned ${response.status}: ${body}`,
      );
    }
    ...
    const hasHttpMethod = latestEvent.tags.some(
      (tag) => tag.key === "http.method",
    );

    if (hasHttpMethod) {
      const methodTag = latestEvent.tags.find(
        (tag) => tag.key === "http.method",
      );
      console.log(
        `✅ Tag found: http.method = "${methodTag?.value}"`,
      );
    } else {
      console.log(`   Available tags: ${latestEvent.tags.map((t) => t.key).join(", ") || "none"}`);
    }
...
async function main(): Promise<void> {
  console.log("🎯 BH1 Sentry Event Trigger & Tag Verification");
  ...
  if (successCount < EVENT_COUNT) {
    ...
    console.error("   BH1 bug may not be active or the endpoint is unreachable.");
    process.exit(1);
  }
  ...
  if (!hasCreds) {
    ...
    console.error(
      "   Without verification, we cannot confirm the incident will be routed correctly.",
    );
    process.exit(1);
  }
  ...
  if (!tagExists) {
    ...
    console.error(
      "   This means the HTTP incident will silently misroute to Remedy's command harness.",
    );
    console.error("   The BH1 remediation is incomplete or incorrect.",
    );
    process.exit(1);
  }

  // Success
  console.log("\n🎉 SUCCESS: BH1 trigger verified with http.method tag present.");
